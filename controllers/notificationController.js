import { get_notification_service, create_notification_service, is_read_set_true_notification_service, is_read_set_true_for_user_notification_service } from "../services/notificationService.js";
...
export const is_read_set_true_for_user_notification = async (req, res) => {
    try {
        await is_read_set_true_for_user_notification_service(req.body);
        res.status(200).json({ success: true, message: 'Notification updated successfully!.' });
    } catch (error) {
        res.status(200).json({ success: false, message: 'Failed to update notification' });
    }
